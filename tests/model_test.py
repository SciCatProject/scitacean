    ...
    def erase_private(m):
        for field in dataclasses.fields(m):
            if field.name.startswith("_"):
                setattr(m, field.name, None)
        return m

    return st.builds(cls).map(erase_private)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (model.OrigDatablock, model.UploadOrigDatablock),
        (model.Datablock, model.UploadDatablock),
        (model.Technique, model.UploadTechnique),
        (model.Relationship, model.UploadRelationship),
        (model.DataFile, model.UploadDataFile),
    ),
)
...
@settings(max_examples=10)
@given(build_user_model_for_upload(model.Datablock))
def test_upload_datablock_fields(datablock):
    upload_datablock = datablock.make_upload_model()
    assert upload_datablock.chkAlg == datablock.chk_alg
    assert upload_datablock.size == datablock.size
    assert len(upload_datablock.dataFileList) == len(datablock.data_file_list)


@settings(max_examples=10)
@given(st.builds(model.Datablock))
def test_upload_model_rejects_non_upload_fields(datablock):
    datablock._owner_group = "the-owner"
    with pytest.raises(ValueError):
        datablock.make_upload_model()
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (model.OrigDatablock, model.DownloadOrigDatablock),
        (model.Datablock, model.DownloadDatablock),
        ...
        (model.DataFile, model.DownloadDataFile),
    ),
)
...
@settings(max_examples=10)
@given(st.builds(model.DownloadDatablock))
def test_download_datablock_fields(download_datablock):
    datablock = model.Datablock.from_download_model(download_datablock)
    assert datablock.chk_alg == download_datablock.chkAlg
    assert datablock.size == download_datablock.size
    assert len(datablock.data_file_list) == len(download_datablock.dataFileList)
